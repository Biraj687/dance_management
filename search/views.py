...
def search(request):
    ...
    if query:
        ...
        for package in Package.objects.filter(Q(name__icontains=query))[:10]:
            results.append({'type': 'package', 'name': package.name, 'details': package.get_duration_display, 'url': f'/packages/{package.pk}/edit/'})
